...
class windowFor2D():
    ...
    def __init__(self, N, beta_parm, visc_parm, diff_parm, dt_parm, num_of_scenario):
        ...
        self.fluid = mf.Krok( N, visc_parm, diff_parm, dt_parm)
        self.beta = beta_parm
    ...
    def run(self):
        ...
        # POPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP
        # plt.ion()
        if self.num_of_scenario == 1:
            self.fluid.zalanie_scenariusz_1()
        elif self.num_of_scenario == 2:
            self.fluid.zalanie_scenariusz_2()
        elif self.num_of_scenario == 3:
            self.fluid.zalanie_scenariusz_3()
        elif self.num_of_scenario == 4:
            self.fluid.zalanie_scenariusz_4()
        ...
        time = 0
        start = False
        #dens, A, B = self.fluid.krok() #pobiera macierz gęstości
        show_force = False
        while not(game_exit):
            #print time
            time += self.fluid.dt
            #if time > 4:
            if self.num_of_scenario == 1:
                self.fluid.sila_scenariusz_1()
            elif self.num_of_scenario == 2:
                self.fluid.sila_scenariusz_2()
            elif self.num_of_scenario == 3:
                self.fluid.sila_scenariusz_3()
            elif self.num_of_scenario == 4:
                self.fluid.sila_scenariusz_4()

            #if dens[1:66, 1:50].sum() > 0.1:
            if self.num_of_scenario == 1:
                dens, A, B = self.fluid.krok2()
            else:
                dens, A, B = self.fluid.krok()
